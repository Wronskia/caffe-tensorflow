...
def display_results(image_paths, probs):
    ...
    print("class_indices =")
    print(class_indices)
    class_indices2=np.argsort(probs)[-5:]
    ...
    for img_idx, image_path in enumerate(image_paths):
        for x in class_indices2.values():
            print("x=")
            print(x)
            img_name = osp.basename(image_path)
            class_name = class_labels[class_indices2[img_idx]]
            confidence = round(probs[img_idx, class_indices2[img_idx]] * 100, 2)
